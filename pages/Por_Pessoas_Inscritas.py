...
#st.title('Dashboard - Pessoas Inscritas no CadÚnico')
#st.subheader('**Até novembro de 2023*')
st.markdown("""
            <h1 style=' font-size: 3rem;'>
                Dashboard - Pessoas Inscritas no CadÚnico
            </h1>
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------
@st.cache_data
def Carregando_informações():
    df_pessoas = pd.read_csv('https://docs.google.com/spreadsheets/d/e/2PACX-1vQp6ZAqeXzcvTr3NfmpfBKPOSQsctbCYSErqbyS5zzp6NyYzp9z-uT6r1TW83x4LA/pub?gid=999433349&single=true&output=csv', skiprows=3)
    df_pessoas = df_pessoas.iloc[:, [0, 1, 2, 5, 6, 7, 8, 9, 3, 4]]
    df_pessoas['Data'] = pd.to_datetime(df_pessoas['Data'])
    return df_pessoas
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------
@st.cache_data
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------
@st.cache_data
def carregando_informações_população_estado():
...
def Fig_multi_line_chart(df, selected_faixa_, faixa_, selected_estado_):
    ...
    faixas = {
        'pes_ext_pob': {'cor': '#a55462', 'nome': 'Extrema Pobreza'},
        'pes_pob': {'cor': '#a55462', 'nome': 'Pobreza'},
        'pes_baixa_renda': {'cor': '#a55462', 'nome': 'Baixa Renda'},
        'pes_acima_meio_sm': {'cor': '#a55462', 'nome': 'Acima de meio SM'}
    }
...
def Fig_candle_chart(df_, selected_faixa_, faixa_, selected_estado_):
    ...
    fig_candle.update_layout(
        title={
            'text': f'Pessoas inscritas no CadÚnico em <b>{faixa_}</b> por Ano<br><sup><b>{selected_estado_}</sup>',
            'y': 0.95,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top',
        ...
        yaxis_title='<b>Número de Pessoas Inscritas',
        xaxis_title='<b>Ano',
...
def Fig_trend_chart(df_, selected_faixa_, faixa_,  selected_estado_):
    ...
    fig_tendencia.update_layout(
        title={
            'text': f'Tendência: Número de Pessoas Inscritas no CadÚnico em {faixa_} - {selected_estado_}',
            'x': 0.5,
            'y': 0.9,
            'xanchor': 'center',
            'yanchor': 'top'
...
def plot_single_graph(df, selected_year, selected_faixa, region_name, color):
    ...
    fig.update_layout(create_graph_layout(
        f"Pessoas Inscritas no CadÚnico em <b>{selected_faixa}</b><br>- <b>{region_name}</b> - {selected_year}",
        selected_faixa,
        y1_color=color
    ))
...
def plot_multi_graph(df, selected_year, selected_faixas, region_name):
    ...
    fig.update_layout(create_graph_layout(
        f"Pessoas Inscritas no CadÚnico em <b>{selected_faixas[0]}</b> e <b>{selected_faixas[1]}</b><br>- <b>{region_name}</b> - {selected_year}",
        selected_faixas[0],
        selected_faixas[1],
        colors[0],
        colors[1]
...
dict_faixa = {'Extrema Pobreza': 'pes_ext_pob', 'Pobreza': 'pes_pob', 'Baixa Renda': 'pes_baixa_renda', "Até meio salário-mínimo": 'pes_acima_meio_sm'}

df_cadúnico = Carregando_informações()
...
with tab_panorama:
    ...
    def processar_dados(selected_year, selected_aggregation, selected_faixa, tipo_de_gráfico):
        df_media_fam = carregando_informações_população_estado()
        ...
        df_combined['diff'] = (df_combined[selected_faixa] / df_combined['População'])*100
    ...
    def criar_chart_title(selected_view, faixa, selected_year):
        if selected_view == 'Média do ano':
            return f"Média de Pessoas inscritas no CadÚnico em <b>{faixa} por estado</b><br>{selected_year}"
        return f"Pessoas inscritas no CadÚnico em <b>{faixa} por estado</b><br>Último mês de referência - {selected_year}"
    #-----------------------------------------------------------------------------------------------------------------------
    def Fig_panorama_chart(df_combined, selected_estado, selected_faixa, faixa, panorama_titulo, tipo_de_gráfico):
        ...
        if tipo_de_gráfico == 'Proporção':
            ...
            fig.update_layout(
                ...
                legend_title_text='Legenda',
                title={
                    'text': f'Diferença percentual entre o número de Pessoas em {faixa} e a população do estado.',
                    'x': 0.5,
                    'y': 0.95,
                    'xanchor': 'center',
                    'yanchor': 'top'
    ...
    with st.container(border=True):
        ...
        with st.container(border=True):
            ...
            fig_sazonalidade.update_layout(
                xaxis_title='Mês',
                yaxis_title='Número médio de Pessoas Inscritas no CadÚnico',
            ...
            with col_candle:
                ...
                with col_crescimento:
                    ...
                    def df_pizza(_df, selected_v):
                        df_pizza = _df.loc[_df['Ano'] == 2023].iloc[-1]
                        
                        cols = ['pes_ext_pob', 'pes_pob', 'pes_baixa_renda', 'pes_acima_meio_sm']
                        ...
                        fig.update_layout(
                            title=dict(
                                text=f'Pessoas Inscritas no CadÚnico por Faixa de Renda - {selected_estado}<br>Último mês de referência.',
                                font=dict(size=20),
                                x=0.5,
                                xanchor='center'
                            ),
